...
// This Visitor is a pattern which is used to inherit the call operator from
// several other classes. Usually this is used, including in this file to
// inherit from several lambdas. For more see:
...
rclcpp::Time getTimestamp(const sensor_msgs::msg::Image::ConstSharedPtr& msg) {
...
NvbloxNode::NvbloxNode(const rclcpp::NodeOptions& options,
  ...
  layer_publisher_ = std::make_unique<LayerPublisher>(
      params_.mapping_type, params_.layer_visualization_min_tsdf_weight,
      params_.layer_visualization_exclusion_height_m,
      params_.layer_visualization_exclusion_radius_m, this);

  RCLCPP_INFO_STREAM(get_logger(),
                     "Create nvblox cuda stream with type: "
  ...
  RCLCPP_INFO_STREAM(get_logger(), "Started up nvblox node in frame "
                                       << params_.global_frame.get()
                                       << " and voxel size "
                                       << params_.voxel_size);

  // Check if a valid mapping typ was selected
  RCLCPP_INFO_STREAM(get_logger(),
                     "Mapping type: " << toString(params_.mapping_type));

  // Output debug info
  CHECK(parameter_tree_.children().has_value());
  parameter_tree_.children().value().push_back(
      multi_mapper_->getParameterTree());
  RCLCPP_INFO_STREAM(
      get_logger(),
      "nvblox parameters:\n"
...
NvbloxNode::~NvbloxNode() {
  if (params_.after_shutdown_map_save_path.get().size() > 0) {
    ...
    esdf_slice_converter_.sliceLayerToDistanceImage(
        ...
        static_mapper_->esdf_integrator().esdf_slice_height(),
    ...
    if (width == 0 || height == 0) {
      ...
      std::string png_path =
          params_.after_shutdown_map_save_path.get() + ".png";
      nvblox::conversions::saveOccupancyGridAsPng(png_path, kFreeThreshold,
                                                  kOccupiedThreshold, height,
                                                  width, occupancy_grid);
      RCLCPP_INFO_STREAM(get_logger(),
                         "Writing occupancy map to: " << png_path);

      const int file_name_index = png_path.find_last_of("/\\");
      std::string image_name = png_path.substr(file_name_index + 1);
      std::string yaml_path =
          params_.after_shutdown_map_save_path.get() + ".yaml";
      nvblox::conversions::saveOccupancyGridYaml(
          yaml_path, image_name, static_mapper_->esdf_layer().voxel_size(),
          aabb.min().x(), aabb.min().y(), kFreeThreshold, kOccupiedThreshold);
      RCLCPP_INFO_STREAM(
          get_logger(),
          "Writing occupancy map yaml meta data to: " << yaml_path);
    }
  }

  // Having the destructor destroying NITROS types may fail if the process-wide
  // GXF/NITROS context has been released elsewhere. This is out of control of
  // this node. To avoid crashes, we refrain from deleting them and instead
  // relay on the OS for memory cleanup. Note that this will lead to memory
  // leaks if several classes are instantiated in the same process.
...
void NvbloxNode::subscribeToTopics() {
  ...
  pointcloud_queue_ = std::make_unique<
      std::list<sensor_msgs::msg::PointCloud2::ConstSharedPtr>>();
  esdf_service_queue_ = std::make_unique<std::list<EsdfServiceQueuedType>>();
  file_path_service_queue_ =
      std::make_unique<std::list<FilePathServiceQueuedType>>();
  ...
  const rclcpp::QoS input_qos =
      AddQosParameter(*this, kDefaultInputQos_, "input_qos");
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     "Subscribing input topics with QoS: " << input_qos_str);
      ...
      if (params_.use_segmentation) {
        ...
        timesync_depth_mask_.emplace_back(
            std::make_shared<image_mask_approx_sync>(
                ...
                *segmentation_image_subs_.back(),
                *segmentation_camera_info_subs_.back()));

        timesync_depth_mask_.back()->registerCallback(
            std::bind(&NvbloxNode::depthPlusMaskImageCallback, this,
                      std::placeholders::_1, std::placeholders::_2,
                      std::placeholders::_3, std::placeholders::_4));
      } else {
        // Sync only depth image with its camera info
        timesync_depth_.emplace_back(std::make_shared<image_exact_sync>(
            image_exact_sync(kQueueSize), *depth_image_subs_.back(),
            *depth_camera_info_subs_.back()));

        timesync_depth_.back()->registerCallback(
            std::bind(&NvbloxNode::depthImageCallback, this,
                      std::placeholders::_1, std::placeholders::_2));
      }
    }
  }
    ...
    for (int i = camera_index; i < params_.num_cameras; ++i) {
      ...
      if (params_.use_segmentation) {
        ...
        timesync_color_mask_.back()->registerCallback(
            std::bind(&NvbloxNode::colorPlusMaskImageCallback, this,
                      std::placeholders::_1, std::placeholders::_2,
                      std::placeholders::_3, std::placeholders::_4));
      } else {
        // Sync only color image with its camera info
        timesync_color_.emplace_back(std::make_shared<image_exact_sync>(
            image_exact_sync(kQueueSize), *color_image_subs_.back(),
            *color_camera_info_subs_.back()));

        timesync_color_.back()->registerCallback(
            std::bind(&NvbloxNode::colorImageCallback, this,
                      std::placeholders::_1, std::placeholders::_2));
      }
    }
  }
...
void NvbloxNode::advertiseTopics() {
  ...
  static_map_slice_publisher_ =
      create_publisher<nvblox_msgs::msg::DistanceMapSlice>("~/static_map_slice",
                                                           1);

  static_occupancy_grid_publisher_ =
      create_publisher<nav_msgs::msg::OccupancyGrid>("~/static_occupancy_grid",
                                                     1);
  dynamic_occupancy_grid_publisher_ =
      create_publisher<nav_msgs::msg::OccupancyGrid>("~/dynamic_occupancy_grid",
                                                     1);
  combined_occupancy_grid_publisher_ =
      create_publisher<nav_msgs::msg::OccupancyGrid>(
          "~/combined_occupancy_grid", 1);

  // Debug outputs
  esdf_slice_bounds_publisher_ =
      create_publisher<visualization_msgs::msg::Marker>("~/esdf_slice_bounds",
                                                        1);
  workspace_bounds_publisher_ =
      create_publisher<visualization_msgs::msg::Marker>("~/workspace_bounds",
                                                        1);
...
void NvbloxNode::advertiseServices() {
  ...
  save_rates_service_ = create_service<nvblox_msgs::srv::FilePath>(
      "~/save_rates",
      std::bind(&NvbloxNode::saveRates, this, std::placeholders::_1,
                std::placeholders::_2),
      service_qos);
  save_timings_service_ = create_service<nvblox_msgs::srv::FilePath>(
      "~/save_timings",
      std::bind(&NvbloxNode::saveTimings, this, std::placeholders::_1,
                std::placeholders::_2),
      service_qos);
  send_esdf_and_gradient_service_ =
      create_service<nvblox_msgs::srv::EsdfAndGradients>(
          "~/get_esdf_and_gradient",
          std::bind(&NvbloxNode::getEsdfAndGradientService, this,
                    std::placeholders::_1, std::placeholders::_2),
          service_qos);
}
...
void NvbloxNode::depthPlusMaskImageCallback(
  ...
  pushOntoQueue<ImageTypeVariant>(
      "depth_queue",
      std::make_tuple(depth_image, depth_camera_info, seg_image,
                      seg_camera_info),
...
void NvbloxNode::colorPlusMaskImageCallback(
  ...
  pushOntoQueue<ImageTypeVariant>(
      "color_mask_queue",
      std::make_tuple(color_image, color_camera_info, seg_image,
                      seg_camera_info),
...
void NvbloxNode::pointcloudCallback(
  ...
  timing::Delays::tick("ros/pointcloud_callback",
                       nvblox::Time(pointcloud_stamp.nanoseconds()),
                       nvblox::Time(now().nanoseconds()));

  pushOntoQueue(kPointcloudQueueName, pointcloud, pointcloud_queue_,
                &pointcloud_queue_mutex_);
}

bool NvbloxNode::shouldProcess(const rclcpp::Time& time_now,
...
void NvbloxNode::tick() {
  ...
  // Process sensor data
  // NOTE: We process these queues every time, checking if we can process (or
  // discard) messages in the queue. Dropping messages in order to not exceed
  // integration rates is handled inside the processQueue functions.
  if (params_.use_depth) {
    processDepthQueue();
  }
  if (params_.use_color) {
  ...
  if (const rclcpp::Time now = this->get_clock()->now();
      ...
      shouldProcess(now, decay_dynamic_occupancy_last_time_,
  ...
  // Mapping
  if (const rclcpp::Time now = this->get_clock()->now();
      shouldProcess(now, clear_map_outside_radius_last_time_,
                    params_.clear_map_outside_radius_rate_hz)) {
  ...
  if (const rclcpp::Time now = this->get_clock()->now(); shouldProcess(
          now, publish_layer_last_time_, params_.publish_layer_rate_hz)) {
    publishLayers();
    publish_layer_last_time_ = now;
  }

  if (const rclcpp::Time now = this->get_clock()->now();
      shouldProcess(now, publish_debug_vis_last_time_,
                    params_.publish_debug_vis_rate_hz)) {
  ...
  if (params_.print_timings_to_console) {
    RCLCPP_INFO_STREAM_THROTTLE(get_logger(), clk,
                                params_.print_statistics_on_console_period_ms,
                                "Timing statistics: \n"
                                    << nvblox::timing::Timing::Print());
  }
  ...
  return std::visit(
      Visitor{// Image
              [this](const ImageMsgTuple& msg) -> bool {
                ...
                return this->canTransform(img_msg->header.frame_id,
                                          getTimestamp(img_msg));
              },
              // Image + Mask
              [this](const ImageSegmentationMaskMsgTuple& msg) -> bool {
                ...
                return this->canTransform(img_msg->header.frame_id,
                                          getTimestamp(img_msg)) &&
                       this->canTransform(mask_msg->header.frame_id,
                                          getTimestamp(mask_msg));
              }},
      variant_msg);
}
...
void NvbloxNode::processDepthQueue() {
  ...
  processQueue<ImageTypeVariant>(depth_image_queue_,   // NOLINT
...
void NvbloxNode::processColorQueue() {
  ...
  processQueue<ImageTypeVariant>(color_image_queue_,   // NOLINT
...
void NvbloxNode::processPointcloudQueue() {
  ...
  processQueue<PointcloudMsg>(pointcloud_queue_,         // NOLINT
                              &pointcloud_queue_mutex_,  // NOLINT
                              message_ready,             // NOLINT
                              std::bind(&NvbloxNode::processLidarPointcloud,
                                        this, std::placeholders::_1));
}

void NvbloxNode::processServiceRequestTaskQueue() {
  // Services are always ready to be processed.
  auto service_ready = [](auto&) -> bool { return true; };
  ...
  processQueue<EsdfServiceQueuedType>(
      esdf_service_queue_, &esdf_service_queue_mutex_, service_ready, task);
  processQueue<FilePathServiceQueuedType>(file_path_service_queue_,
  ...
  if (params_.esdf_mode == EsdfMode::k2D) {
    ...
    if (isUsingHumanOrDynamicMapper(params_.mapping_type)) {
      ...
      sliceAndPublishEsdf(
          ...
          combined_esdf_pointcloud_publisher_, combined_map_slice_publisher_,
          combined_occupancy_grid_publisher_,
          params_.distance_map_unknown_value_optimistic, dynamic_mapper_.get());
    }

    if (params_.output_pessimistic_distance_map) {
      sliceAndPublishEsdf("pessimistic_static", static_mapper_,
                          pessimistic_static_esdf_pointcloud_publisher_,
...
void NvbloxNode::sliceAndPublishEsdf(
  ...
  if (pointcloud_publisher->get_subscription_count() > 0 ||
      (params_.publish_esdf_distance_slice &&
       slice_publisher->get_subscription_count() > 0) ||
      (occupancy_grid_publisher &&
       occupancy_grid_publisher->get_subscription_count() > 0)) {
    ...
    if (mapper_2 != nullptr) {
      esdf_slice_converter_.sliceLayersToCombinedDistanceImage(
          ...
          mapper_2->esdf_integrator().esdf_slice_height(), unknown_value,
          &map_slice_image, &aabb);
    } else {
      esdf_slice_converter_.sliceLayerToDistanceImage(
          mapper->esdf_layer(), mapper->esdf_integrator().esdf_slice_height(),
          unknown_value, &map_slice_image, &aabb);
    }
    slicing_timer.Stop();
    ...
    if (pointcloud_publisher->get_subscription_count() > 0) {
      ...
      esdf_slice_converter_.pointcloudMsgFromSliceImage(
          ...
          mapper->esdf_layer().voxel_size(), unknown_value, &pointcloud_msg);
    ...
    if (params_.publish_esdf_distance_slice &&
        slice_publisher->get_subscription_count() > 0) {
      ...
      esdf_slice_converter_.distanceMapSliceMsgFromSliceImage(
          map_slice_image, aabb, mapper->esdf_integrator().esdf_slice_height(),
          mapper->voxel_size_m(), unknown_value, &map_slice_msg);
    ...
    if (occupancy_grid_publisher &&
        occupancy_grid_publisher->get_subscription_count() > 0) {
      timing::Timer pointcloud_msg_timer("ros/" + name +
                                         "/esdf/output/occupancy_grid");

      const int width = map_slice_image.cols();
      const int height = map_slice_image.rows();

      if (width > 0 && height > 0) {
        publishOccupancyGridMsg(mapper->esdf_layer().voxel_size(), width,
                                height, aabb.min().x(), aabb.min().y(),
                                map_slice_image, unknown_value,
                                occupancy_grid_publisher);
      }
    }
  }
...
void NvbloxNode::publishOccupancyGridMsg(
    const float voxel_size, const int width, const int height,
    const double origin_x_position, const double origin_y_position,
    const nvblox::Image<float>& map_slice_image, const float unknown_value,
    const rclcpp::Publisher<nav_msgs::msg::OccupancyGrid>::SharedPtr&
        occupancy_grid_publisher) {
  // Set up the message
  ...
  esdf_slice_converter_.occupancyGridFromSliceImage(
      map_slice_image, occupancy_grid_msg.data.data(), unknown_value);
...
bool NvbloxNode::canTransform(const std::string& frame_id,
...
NvbloxNode::ImageMsgOptionalMaskMsgTuple NvbloxNode::decomposeImageTypeVariant(
  ...
  std::visit(Visitor{// Image
                     ...
                     [&](const ImageSegmentationMaskMsgTuple& msg) {
                       ...
                       mask_camera_info_opt =
                           std::get<kMsgTupleMaskCameraInfoIdx>(msg);
                     }},
             variant_msg);
...
bool NvbloxNode::processDepthImage(const ImageTypeVariant& depth_msg) {
  ...
  if (integrate_depth_last_times_.find(depth_frame) ==
      integrate_depth_last_times_.end()) {
    integrate_depth_last_times_[depth_frame] = rclcpp::Time(0, 0, RCL_ROS_TIME);
  }
  const rclcpp::Time last_depth_image_timestamp =
      integrate_depth_last_times_[depth_frame];
  if (!shouldProcess(depth_image_timestamp, last_depth_image_timestamp,
                     params_.integrate_depth_rate_hz)) {
    // To discard we indicate that the image was processed, without actually
    // integrating it.
    return true;
  }
  if (depth_image_timestamp <= shape_clearing_last_time_) {
    // Discard images older then the last shape clearing (would reintegrate
    // cleared shapes).
    return true;
  }
  // Get the timestamp
  ...
  if (!transformer_.lookupTransformToGlobalFrame(
          depth_frame, depth_image_timestamp, &T_L_C_depth_)) {
    return false;
  }
  ...
  if (mask_img_opt) {
    if (!transformer_.lookupTransformToGlobalFrame(
            mask_frame, mask_image_timestamp, &T_L_C_mask)) {
      return false;
    }
  ...
  const Camera depth_camera_ =
      conversions::cameraFromMessage(*depth_camera_info_msg);
  Camera mask_camera;
  if (mask_img_opt) {
  ...
  if (mask_img_opt) {
    if (!conversions::monoImageFromImageMessageAsync(
            *mask_img_opt.value(), &mask_image_, *cuda_stream_)) {
  ...
  if (mask_img_opt) {
    ...
    multi_mapper_->integrateDepth(depth_image_, T_L_C_depth_, depth_camera_,
                                  update_time_ms);
  }
  timing::Delays::tick("ros/depth_image_integration",
  ...
  newest_integrated_depth_time_ =
      std::max(depth_image_timestamp, newest_integrated_depth_time_);
  integration_timer.Stop();
  // Optional publishing - Freespace + Dynamics
  if (isDynamicMapping(params_.mapping_type)) {
    // Process the freespace layer
    // TODO(dtingdahl) Move this into the publishLayers() function so we publish
    // visualization messages at the same place (and separated from processing)
  ...
  if (back_projected_depth_publisher_[depth_frame]->get_subscription_count() >
      0) {
    // Only back project and publish every n-th depth image
    if (back_projection_idx_[depth_frame]++ %
            params_.back_projection_subsampling ==
        0) {
      publishBackProjectedDepth(depth_camera_, depth_frame,
...
void NvbloxNode::publishDynamics(const std::string& camera_frame_id) {
  ...
  if (dynamic_points_publisher_->get_subscription_count() > 0) {
    ...
    pointcloud_converter_.pointcloudMsgFromPointcloud(dynamic_points,
                                                      &dynamic_points_msg);
  ...
  if (dynamic_depth_frame_overlay_publisher_->get_subscription_count() > 0) {
    ...
    conversions::imageMessageFromColorImage(dynamic_overlay, camera_frame_id,
                                            &img_msg, *cuda_stream_);
    dynamic_depth_frame_overlay_publisher_->publish(img_msg);
  }
}

void NvbloxNode::publishHumanDebugOutput(const std::string& camera_frame_id,
                                         const Camera& camera) {
  ...
  if (dynamic_points_publisher_->get_subscription_count() > 0) {
    ...
    const DepthImage& depth_image_only_humans =
        multi_mapper_->getLastDepthFrameForeground();
    // Back project
  ...
  if (dynamic_depth_frame_overlay_publisher_->get_subscription_count() > 0) {
    ...
    conversions::imageMessageFromColorImage(depth_overlay, camera_frame_id,
                                            &img_msg, *cuda_stream_);
    dynamic_depth_frame_overlay_publisher_->publish(img_msg);
  }
}

void NvbloxNode::publishBackProjectedDepth(const Camera& camera,
                                           const std::string& frame,
                                           const rclcpp::Time& timestamp) {
  // Get the pointcloud from the depth image
  image_back_projector_.backProjectOnGPU(depth_image_, camera,
                                         &pointcloud_C_device_,
                                         params_.max_back_projection_distance);

  // Send the message
  sensor_msgs::msg::PointCloud2 back_projected_depth_msg;
  pointcloud_converter_.pointcloudMsgFromPointcloud(pointcloud_C_device_,
...
bool NvbloxNode::processColorImage(const ImageTypeVariant& color_msg) {
  ...
  if (integrate_color_last_times_.find(color_frame) ==
      integrate_color_last_times_.end()) {
    integrate_color_last_times_[color_frame] = rclcpp::Time(0, 0, RCL_ROS_TIME);
  }
  const rclcpp::Time last_color_image_timestamp =
      integrate_color_last_times_[color_frame];
  if (!shouldProcess(color_image_timestamp, last_color_image_timestamp,
  ...
  Transform T_L_C;
  if (!transformer_.lookupTransformToGlobalFrame(
          color_frame, color_image_timestamp, &T_L_C)) {
    return false;
  }
  Transform T_L_C_mask;
  if (mask_img_opt) {
  ...
  // Convert camera info message to camera object.
  const Camera color_camera =
      conversions::cameraFromMessage(*color_camera_info_msg);

  // Convert the color image.
  if (!conversions::colorImageFromImageMessageAsync(
          *color_img_ptr, &color_image_, &rgb_image_tmp_, &bgra_image_tmp_,
  ...
  timing::Delays::tick("ros/color_image_integration",
...
bool NvbloxNode::processLidarPointcloud(
  ...
  if (!shouldProcess(pointcloud_timestamp, integrate_lidar_last_time_,
  ...
  Lidar lidar = (params_.use_non_equal_vertical_fov_lidar_params)
                    ? Lidar(params_.lidar_width, params_.lidar_height,
                            params_.lidar_min_valid_range_m,
                            params_.lidar_max_valid_range_m,
  ...
  timing::Delays::tick("ros/pointcloud_integration",
                       nvblox::Time(pointcloud_timestamp.nanoseconds()),
                       nvblox::Time(now().nanoseconds()));
  newest_integrated_depth_time_ =
      std::max(pointcloud_timestamp, newest_integrated_depth_time_);
...
void NvbloxNode::publishLayers() {
  ...
  Transform T_L_C;
  if (!transformer_.lookupTransformToGlobalFrame(params_.map_clearing_frame_id,
                                                 rclcpp::Time(0), &T_L_C)) {
  ...
  layer_publisher_->serializeAndpublishSubscribedLayers(
      T_L_C, frame_id, timestamp, params_.layer_streamer_bandwidth_limit_mbps,
      static_mapper_, dynamic_mapper_, get_logger());
}
...
void NvbloxNode::publishDebugVisualizations() {
  ...
  if (tsdf_zero_crossings_pointcloud_publisher_->get_subscription_count() > 0) {
    std::optional<std::vector<Vector3f>> maybe_tsdf_zero_crossings =
        multi_mapper_->ground_plane_estimator()
            .tsdf_zero_crossings_ground_candidates();
    if (maybe_tsdf_zero_crossings) {
      ...
      tsdf_zero_crossings_pointcloud2_msg.header.frame_id =
          params_.global_frame.get();
  ...
  if (tsdf_zero_crossings_ground_plane_publisher_->get_subscription_count() >
      0) {
    std::optional<Plane> maybe_plane =
        multi_mapper_->ground_plane_estimator().ground_plane();
    if (maybe_plane) {
      Transform T_S_PB;
      if (transformer_.lookupTransformToGlobalFrame(
        ...
        const visualization_msgs::msg::Marker marker_plane =
            planeToMarker(T_S_PB, maybe_plane.value(),
  ...
  if (esdf_slice_bounds_publisher_->get_subscription_count() > 0) {
    ...
    Transform T_S_PB;
    if (transformer_.lookupTransformToGlobalFrame(
            params_.esdf_slice_bounds_visualization_attachment_frame_id,
            rclcpp::Time(0), &T_S_PB)) {
      ...
      const visualization_msgs::msg::Marker marker_bottom = heightLimitToMarker(
          T_S_PB, params_.esdf_slice_bounds_visualization_side_length,
          timestamp, params_.global_frame.get(),
          static_mapper_->esdf_integrator().esdf_slice_min_height(),
          HeightLimitMarkerType::kBottomHeightLimit);
      const visualization_msgs::msg::Marker marker_top = heightLimitToMarker(
          T_S_PB, params_.esdf_slice_bounds_visualization_side_length,
          timestamp, params_.global_frame.get(),
          static_mapper_->esdf_integrator().esdf_slice_max_height(),
  ...
  // Workspace Bounds
  if (workspace_bounds_publisher_ &&
      workspace_bounds_publisher_->get_subscription_count() > 0) {
    // Get the workspace bounds type and corners.
    const WorkspaceBoundsType bounds_type = static_mapper_->tsdf_integrator()
                                                .view_calculator()
                                                .workspace_bounds_type();
    const Vector3f min_corner = static_mapper_->tsdf_integrator()
                                    .view_calculator()
                                    .workspace_bounds_min_corner_m();
    const Vector3f max_corner = static_mapper_->tsdf_integrator()
                                    .view_calculator()
                                    .workspace_bounds_max_corner_m();
    ...
    if (bounds_type == WorkspaceBoundsType::kBoundingBox) {
      const visualization_msgs::msg::Marker workspace_bounds_marker =
          boundingBoxToMarker(min_corner, max_corner, timestamp,
                              params_.global_frame.get());
      workspace_bounds_publisher_->publish(workspace_bounds_marker);
    } else if (bounds_type == WorkspaceBoundsType::kHeightBounds) {
      // The frame to which the workspace height bounds visualization is
      // attached. We get the transform from the plane-body (PB) frame, to the
      // scene (S).
      Transform T_S_PB;
      if (transformer_.lookupTransformToGlobalFrame(
              params_.workspace_height_bounds_visualization_attachment_frame_id,
              rclcpp::Time(0), &T_S_PB)) {
        // Get and publish the planes representing the workspace height bounds
        // in z.
        const visualization_msgs::msg::Marker marker_bottom =
            heightLimitToMarker(
                T_S_PB,
                params_.workspace_height_bounds_visualization_side_length,
                timestamp, params_.global_frame.get(), min_corner.z(),
                HeightLimitMarkerType::kBottomHeightLimit);
        const visualization_msgs::msg::Marker marker_top = heightLimitToMarker(
            T_S_PB, params_.workspace_height_bounds_visualization_side_length,
            timestamp, params_.global_frame.get(), max_corner.z(),
            HeightLimitMarkerType::kTopHeightLimit);
        ...
        RCLCPP_INFO_STREAM_THROTTLE(
            get_logger(), *get_clock(), kTimeBetweenDebugMessagesMs,
            "Tried to publish workspace height bounds but couldn't look up "
            "frame: "
                << params_
...
void NvbloxNode::clearMapOutsideOfRadiusOfLastKnownPose() {
  if (params_.map_clearing_radius_m > 0.0f) {
    ...
    if (transformer_.lookupTransformToGlobalFrame(params_.map_clearing_frame_id,
      ...
      static_mapper_->clearOutsideRadius(T_L_MC.translation(),
                                         params_.map_clearing_radius_m);
    } else {
      RCLCPP_INFO_STREAM_THROTTLE(
          get_logger(), *get_clock(), kTimeBetweenDebugMessagesMs,
...
void NvbloxNode::savePly(
  ...
  TaskFunctionType<NvbloxNode, nvblox_msgs::srv::FilePath> request_task =
      [](auto node, auto service_request, auto service_response) {
        ...
        if (success) {
          RCLCPP_INFO_STREAM(
              node->get_logger(),
              "Output PLY file(s) to " << service_request->file_path);
        } else {
          RCLCPP_WARN_STREAM(
              node->get_logger(),
              "Failed to write PLY file(s) to " << service_request->file_path);
        }
  ...
  auto task = std::make_shared<
      ServiceRequestTask<NvbloxNode, nvblox_msgs::srv::FilePath>>(
      request_task, this, request, response);

  // Push the task onto the queue and wait for completion.
  pushOntoQueue(kFilePathServiceQueueName, task, file_path_service_queue_,
  ...
  TaskFunctionType<NvbloxNode, nvblox_msgs::srv::FilePath> request_task =
      [](auto node, auto service_request, auto service_response) {
        ...
        service_response->success =
            node->static_mapper_->saveLayerCake(filename);
        if (service_response->success) {
          RCLCPP_INFO_STREAM(node->get_logger(),
                             "Output map to file to " << filename);
        } else {
          RCLCPP_WARN_STREAM(node->get_logger(),
                             "Failed to write file to " << filename);
        }
...
void NvbloxNode::loadMap(
  ...
  TaskFunctionType<NvbloxNode, nvblox_msgs::srv::FilePath> request_task =
      [](auto node, auto service_request, auto service_response) {
        ...
        if (service_response->success) {
          RCLCPP_INFO_STREAM(node->get_logger(),
                             "Loaded map to file from " << filename);
        } else {
          RCLCPP_WARN_STREAM(node->get_logger(),
...
void NvbloxNode::getEsdfAndGradientService(
  ...
  RCLCPP_INFO(get_logger(),
              ...
              request->update_esdf, request->use_aabb, request->visualize_esdf,
              request->aabb_min_m.x, request->aabb_min_m.y,
              request->aabb_min_m.z, request->aabb_size_m.x,
              request->aabb_size_m.y, request->aabb_size_m.z);
  ...
  // Define the task function
  TaskFunctionType<NvbloxNode, nvblox_msgs::srv::EsdfAndGradients>
      request_task = [](auto node, auto service_request,
        ...
        if (service_request->frame_id != node->params_.global_frame.get()) {
          RCLCPP_WARN_STREAM(node->get_logger(),
                             "Requested EsdfAndGradients in "
                                 << service_request->frame_id
                                 << " frame but nvblox is mapping in "
                                 << node->params_.global_frame.get()
                                 << " frame. Sending empty grid.");
          service_response->success = false;
          return false;
        }

        // Clear shapes in the tsdf and visualize them in rviz if requested.
        timing::Timer clear_shapes_timer(
            "ros/esdf_service/task_function/clear_shapes");
        ...
        if (node->shapes_to_clear_publisher_ &&
          ...
          timing::Timer clear_shapes_vis_timer(
              "ros/esdf_service/task_function/clear_shapes/vis");
          const visualization_msgs::msg::MarkerArray marker_array =
              boundingShapesToMarker(shapes_to_clear, node->get_clock()->now(),
                                     node->params_.global_frame.get(),
                                     node->get_logger());
        ...
        if (service_request->update_esdf) {
          timing::Timer update_esdf_timer(
              "ros/esdf_service/task_function/update_esdf");
          node->processEsdf();
        ...
        timing::Timer create_response_timer(
            "ros/esdf_service/task_function/create_response");
        node->esdf_and_gradients_converter_.getEsdfAndGradientResponse(
            node->static_mapper_->esdf_layer(),
            node->params_.esdf_and_gradients_unobserved_value, service_request,
            service_response, *node->cuda_stream_);
        ...
        if (service_response->success) {
          ...
          RCLCPP_WARN_STREAM(node->get_logger(),
                             "Writing requested ESDF to MultiArrayMsg failed. "
                             "Is your AABB definition valid?");
        }
  ...
  // Create the ServiceRequestTask
  auto task = std::make_shared<
      ServiceRequestTask<NvbloxNode, nvblox_msgs::srv::EsdfAndGradients>>(
      request_task, this, request, response);

  // Push the task onto the queue and wait for completion.
  pushOntoQueue(kEsdfServiceQueueName, task, esdf_service_queue_,
                &esdf_service_queue_mutex_);
  task->waitForTaskCompletion();
}
